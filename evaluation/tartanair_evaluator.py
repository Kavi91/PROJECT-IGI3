...
class TartanAirEvaluator:
    ...
    def evaluate_one_trajectory(self, gt_traj_name, est_traj_name, scale=False):
        ...
        try:
            gt_traj = np.loadtxt('pose_gt.txt')
            est_traj = np.loadtxt('pose_est.txt')
            print("GT shape:", gt_traj.shape)  # Should be (N, 7)
            print("EST shape:", est_traj.shape)  # Should be (M, 7)
            
            # Print first few rows to check format
            print("GT first rows:\n", gt_traj[:2])
            print("EST first rows:\n", est_traj[:2])
